

-------------------- City
-- GET https://nscrail.himsportal.com/api/City/GetAllCities
GET https://nscrail.himsportal.com/api/City/Get/{id}
-- POST https://nscrail.himsportal.com/api/City
-- PUT https://nscrail.himsportal.com/api/City/{id}
-- DELETE https://nscrail.himsportal.com/api/City/{id}


------------------ Station
-- GET https://nscrail.himsportal.com/api/Station/GetAllStations
GET https://nscrail.himsportal.com/api/Station/Get/{id}
-- POST https://nscrail.himsportal.com/api/Station
-- PUT https://nscrail.himsportal.com/api/Station/{id}
-- DELETE https://nscrail.himsportal.com/api/Station/{id}


------------------- Train
-- GET https://nscrail.himsportal.com/api/Train/GetAllTrains
GET https://nscrail.himsportal.com/api/Train/Get/{id}
-- POST https://nscrail.himsportal.com/api/Train
-- PUT https://nscrail.himsportal.com/api/Train/{id}
-- DELETE https://nscrail.himsportal.com/api/Train/{id}


------------------- Route
-- GET https://nscrail.himsportal.com/api/Route/GetAllRoutes
-- GET https://nscrail.himsportal.com/api/Route/Get/{id}
-- POST https://nscrail.himsportal.com/api/Route
-- PUT https://nscrail.himsportal.com/api/Route/{id}
-- DELETE https://nscrail.himsportal.com/api/Route/{id}


-- ------------------Train Coaches
-- GET https://nscrail.himsportal.com/api/TrainCoach/GetAllCoaches
-- GET https://nscrail.himsportal.com/api/TrainCoach/Get/{id}
-- POST https://nscrail.himsportal.com/api/TrainCoach
PUT https://nscrail.himsportal.com/api/TrainCoach
-- DELETE https://nscrail.himsportal.com/api/TrainCoach/{id}









